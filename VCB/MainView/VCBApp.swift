...
var statusMenu: NSMenu = NSMenu() // 状态菜单
var statusBarItem: NSStatusItem! // 状态选项
var mouseMonitor: Any? // 鼠标监视器
var keyMonitor: Any? // key监视器
var hideMousePointer = false // 因残鼠标指针
var hideScreenMagnifier = false // 隐藏屏幕放大镜
...
//     注册全局的鼠标监视器
    ...
    func applicationWillTerminate(_ aNotification: Notification) {
        if SCContext.stream != nil { SCContext.stopRecording() } // 停止录制
    }
...
//     这个方法是干嘛的
    ...
    func applicationWillFinishLaunching(_ notification: Notification) {
        let process = NSWorkspace.shared.runningApplications.filter({ $0.bundleIdentifier == "com.helinyu.cvb" })
        if process.count > 1 {
            DispatchQueue.main.async {
                let button = self.createAlert(title: "VCB is Running".local, message: "Please do not run multiple instances!".local, button1: "Quit".local).runModal()
                if button == .alertFirstButtonReturn { NSApp.terminate(self) }
            }
        }
        
        lazy var downloadDir = (NSSearchPathForDirectoriesInDomains(.downloadsDirectory, .userDomainMask, true) as [String]).first!
        downloadDir.append("/Vcb")
//      默认目录是： downloads/Vcb
...
extension NSImage {
    static func createScreenShot() -> NSImage? {
        let excludedAppBundleIDs = ["com.helinyu.vcb"]
